document.addEventListener("DOMContentLoaded", function () {
  ...
  function getPingAngles() {
    return Array.from(pings).map(ping => ({
      el: ping,
      angle: getAngle(ping)
    }));
  }

  let pingAngles = getPingAngles();

  window.addEventListener("resize", () => {
    pingAngles = getPingAngles(); // recalculate on resize
  });
  ...
  function updateSweep() {
    ...
    pingAngles.forEach(({ el, angle }) => {
      ...
      el.style.opacity = angleDiff < 12 ? 1 : 0;
    });
