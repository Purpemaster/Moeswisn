document.addEventListener("DOMContentLoaded", function () {
  const sweepDuration = 6000; // duration in ms (must match CSS)
  ...
  function calculatePingAngles() {
    pingAngles = Array.from(pings).map(ping => ({
      el: ping,
      angle: getAngle(ping)
    }));
  }
  ...
  // Make calculatePingAngles available to global scope
  window.calculatePingAngles = calculatePingAngles;
