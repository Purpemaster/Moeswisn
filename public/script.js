...
async function fetchSolPrice() {
  const now = Date.now();
  if (now - lastPriceFetch < 5 * 60 * 1000 && lastSolPrice !== 0) {
    return lastSolPrice;
  }
...
async function fetchPurpePrice() {
  const now = Date.now();
  if (now - lastPriceFetch < 5 * 60 * 1000 && lastPurpePrice !== 0) {
...
async function fetchBalance() {
  ...
  try {
    const [solRes, tokenRes] = await Promise.all([
      fetch(solUrl, { headers: { accept: "application/json" } }),
...
updateTracker();
setInterval(updateTracker, 30000); // alle 30 Sekunden aktualisieren, aber nur alle 5 Minuten neue Preise holen
