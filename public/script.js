...
const heliusKey = "bf688561-527e-43f9-a24e-add82f281a29";
const purpeTokenAddress = "HBoNJ5v8g71s2boRivrHnfSB5MVPLDHHyVjruPfhGkvL";
...
function setupDonationButtons() {
  ...
  document.getElementById("donate-purpe").onclick = () => {
    window.location.href = `solana:${walletAddress}?amount=3000000&spl-token=${purpeTokenAddress}&label=Purple%20Pepe%20Donation`;
  };
}
...
async function fetchBalance() {
  const url = `https://mainnet.helius.xyz/v1/account/${walletAddress}?api-key=${heliusKey}`;
  try {
    const data = await fetchJson(url);
    if (!data) return { sol: 0, purpe: 0 };

    const sol = data.nativeBalance?.lamports ? data.nativeBalance.lamports / 1e9 : 0;

    let purpe = 0;
    if (Array.isArray(data.tokens)) {
      const purpeToken = data.tokens.find(t => t.tokenAddress === purpeTokenAddress);
      purpe = purpeToken?.amount?.uiAmount || 0;
    }

    console.log("🟣 Balance via Helius:", { sol, purpe });
    return { sol, purpe };
  } catch (err) {
    console.error("💥 Fehler bei fetchBalance (Helius):", err);
