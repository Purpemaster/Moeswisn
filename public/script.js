...
async function updateTracker() {
  const [bal, solPrice, purpePrice] = await Promise.all([fetchBalance(), fetchSolPrice(), fetchPurpePrice()]);
  const total = (bal.sol * solPrice) + (bal.purpe * purpePrice);
  updateProgress(total);
  document.getElementById("last-updated").textContent = new Date().toLocaleTimeString();
}

function setupRadarLogos() {
  const radarContainer = document.querySelector(".radar");
  const logos = [
    { src: "bitmart_logo.png", link: "https://www.bitmart.com/" },
    { src: "slingshot_logo.png", link: "https://slingshot.finance/" },
  ...
  function updateSweep() {
    ...
    const sweepAngle = ((now % 6000) / 6000) * 360;

    logoAngles.forEach(({ el, angle }) => {
      const diff = Math.abs(sweepAngle - angle);
      const angleDiff = Math.min(diff, 360 - diff);
      el.classList.toggle("highlighted", angleDiff < 20);
    });
